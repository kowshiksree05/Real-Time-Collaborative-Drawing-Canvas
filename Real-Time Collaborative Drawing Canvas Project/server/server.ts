...
import { ClientToServerEvents, ServerToClientEvents, DrawingStroke, Point } from './types';
...
app.get('/test', (req, res) => {
  res.json({ 
    ...
    files: ['main.js', 'websocket.js', 'canvas.js'].map(f => {
      const fs = require('fs');
      const filePath = path.join(clientPath, f);
      return {
        file: f,
...
// Store active strokes per user (for drawing in progress)
...
const activeStrokes: Map<string, ActiveStroke> = new Map();
